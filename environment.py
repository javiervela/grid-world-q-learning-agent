...
class Env(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        x_start, y_start = 0, 0
        self.agent_position = canvas.create_image(
            x_start * UNIT + UNIT * 0.5,
            y_start * UNIT + UNIT * 0.5,
            image=self.shapes[0],
        )

        x_penalty_a, y_penalty_a = 0, 2
        self.penalty_a = canvas.create_image(
            x_penalty_a * UNIT + UNIT * 0.5,
            y_penalty_a * UNIT + UNIT * 0.5,
            image=self.shapes[1],
        )
        ...
        self.gem = canvas.create_image(
            x_reward * UNIT + UNIT * 0.5,
            y_reward * UNIT + UNIT * 0.5,
            image=self.shapes[2],
        )
    ...
    def load_images(self):
        ...
        up = PhotoImage(
            Image.open("img/up.png").resize((int(UNIT * 0.13), int(UNIT * 0.13)))
        )
        right = PhotoImage(
            Image.open("img/right.png").resize((int(UNIT * 0.13), int(UNIT * 0.13)))
        )
        left = PhotoImage(
            Image.open("img/left.png").resize((int(UNIT * 0.13), int(UNIT * 0.13)))
        )
        down = PhotoImage(
            Image.open("img/down.png").resize((int(UNIT * 0.13), int(UNIT * 0.13)))
        )

        bot = PhotoImage(
            Image.open("img/bot.png").resize((int(UNIT * 0.65), int(UNIT * 0.65)))
        )
        wrong = PhotoImage(
            Image.open("img/wrong.png").resize((int(UNIT * 0.65), int(UNIT * 0.65)))
        )
        gem = PhotoImage(
            Image.open("img/gem.png").resize((int(UNIT * 0.65), int(UNIT * 0.65)))
        )
        ...
        for i in range(WIDTH):
            for j in range(HEIGHT):
                for action in range(0, 4):
                    state = [i, j]
                    if (
                        state not in self.rewards_pos
                        and state not in self.penalties_pos
                    ):
            ...
            for j in range(HEIGHT):
                ...
                if state not in self.rewards_pos and state not in self.penalties_pos:
                    if str(state) in agent.q_table.keys():
                        best_action, full_draw = agent.arg_max(
                            agent.q_table[str(state)]
                        )
                        if not full_draw:
    ...
    def step(self, action, do_render):
        ...
        if real_action in [0, 1]:
            selection = random.choices(
                [real_action, 2, 3],
                weights=(1 - self.noise, self.noise * 0.5, self.noise * 0.5),
                k=1,
            )
